...
- **Correction to what 0.2.4-alpha claimed.** Its entry and `docs/wire-format.md` §4.3 said "2.1.278 sends typed prompts as plain strings". That is **not** established: a capture of the same 2.1.278 minor under a different beta set sends every typed prompt as an array of blocks, and the current classifier labels all of them `new`. What is proven is narrower — a plain string is at minimum the *history* encoding of a prompt, and one 2.1.278 session (holding `afk-mode`, `advisor-tool`, `dangerous-tool-use` among others) turned 11 typed prompts into 1 `new` turn. §4.3 has been rewritten to separate the proven from the inferred; what triggers the encoding switch is still unknown. The 0.2.4-alpha entry is left as released history.
