...
- **Fix: a user-typed prompt delivered as a plain string is recognised again.** Up to 2.1.277 every typed prompt observed arrived as an array of text blocks and only harness side calls used a plain-string content, so the classifier treated a plain string as proof of a side call. In a 2.1.278 session that held, among others, the `afk-mode`, `advisor-tool` and `dangerous-tool-use` betas, **11 typed `UserPromptSubmit` hooks produced 1 main `new` turn**: each of the ten windows that took no decision contains exactly one `side`/`unclassified` request and no `new` one, arriving 0.0-0.1 s after the user pressed enter with a plain-string last message of 77-254 code points. No decision was taken, the pin never moved, and nothing failed loudly because each request still forwarded unchanged. A separate capture of the **same 2.1.278 minor under a different beta set sent all its typed prompts as arrays**, so the shape is not universal and what triggers it is not yet known. A plain string is now promoted to a `new` turn only when it matches the **newest** typed prompt whose hook has arrived and which no wire turn has claimed — never an older one, because a prompt's identical text is also re-encoded as a plain string once it sits in history (proven by byte comparison), which is the shape a history-replaying side call wears. With no hook stream the request stays `side`, the fail-safe direction. Side markers are still matched first. (`docs/wire-format.md` §4.3, which separates what is proven from what is inferred)
