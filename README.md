...
```sh
npm install -g https://github.com/ziyacivan/reflex-router/releases/download/v0.3.4/reflex-router-0.3.4.tgz
```
...
- **Only verified retargets are applied.** Sonnet → Haiku, Opus → Sonnet, Opus → Haiku, Haiku → Sonnet, Haiku → Opus, Sonnet → Opus. The three upgrades need `REFLEX_UPGRADES=on` or `confident` (off by default) or a `reflex:<tier>` override; Fable retargets are recorded and left alone. Opus → Sonnet and Sonnet → Opus with a reasoning effort other than `medium` are untested.
