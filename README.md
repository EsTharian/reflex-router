...
- **To Anthropic:** your request with your own headers. In `shadow` mode it is byte for byte. In `route` mode a routed request has its model id and dependent fields rewritten (the changed fields are listed in its record), and no text is added, removed or edited. No `REFLEX_*` or `TYPESAFE_*` value ever reaches Anthropic.
