...
- **Side calls keep billing the requested model.** Claude Code's own side calls are never decided and, with one exception, never moved: a routed subagent's progress summaries follow its pin (they replay its history, so they read the cache its loop wrote). Everything else stays on the model you asked for, so part of a routed session's usage does too, by construction. In route session B ([`docs/observations.md`](docs/observations.md)) they also kept that model's prompt cache warm while the conversation itself ran elsewhere.
