...
**Open issue: the session A 400 is unexplained.** Evidence so far: the rejected request was a main-chat first turn, Opus → Haiku, rewritten fields `model, output_config.effort, thinking, messages.system_folded:1`; its error text was not recorded. The reproduction had the same shape and the same rewritten fields, **including one folded `role:"system"` message**, and returned 200. Not yet covered: a folded system message that itself carries a `cache_control` breakpoint with a different TTL from the surrounding ones (the API wants longer-TTL breakpoints before shorter ones); in the reproduction every breakpoint was `1h`. Every fallback record now carries the upstream error (`forwarded.fallback_error`, redacted), so the next occurrence will say what the API rejected.
